...
const extractTxHashFromDescription = (description: string): string | undefined => {
  const match = description?.match(/Transaction Hash: (0x[a-fA-F0-9]{64})/);
  return match?.[1];
};

// Helper function to extract on-chain ID from description
const extractOnChainIdFromDescription = (description: string): string | undefined => {
...
const mapAgreementStatusToEscrow = (status: number): 'pending' | 'active' | 'completed' | 'cancelled' | 'frozen' | 'disputed' => {
  switch (status) {
    case 1: return 'pending'; // PENDING_ACCEPTANCE
    case 2: return 'active'; // ACTIVE
    case 3: return 'completed'; // COMPLETED
    case 4: return 'disputed'; // DISPUTED
    case 5: return 'cancelled'; // CANCELLED
    case 6: return 'cancelled'; // EXPIRED
    default: return 'pending';
  }
};

// Enhanced transform function for escrow agreements
const transformApiAgreementToEscrow = (apiAgreement: any): ExtendedEscrowWithOnChain => {
  ...
  return {
    ...
    deadline: apiAgreement.deadline ? new Date(apiAgreement.deadline).toISOString().split('T')[0] : 'No deadline',
    type: apiAgreement.visibility === 1 ? 'private' : 'public',
    description: apiAgreement.description || '',
    createdAt: new Date(apiAgreement.dateCreated || apiAgreement.createdAt).getTime(),
    // On-chain references
...
export default function Escrow() {
  // ---------- UI initial data (unchanged) ----------
  const initial: ExtendedEscrowWithOnChain[] = useMemo(
    () => [
      {
        id: "E-101",
  ...
  const [escrows, setEscrows] = useState<ExtendedEscrowWithOnChain[]>(initial);
  ...
  const contractAddress = (chainId && ESCROW_CA[chainId as number]) || undefined;
  ...
  const { isSuccess: approvalSuccess } = useWaitForTransactionReceipt({ hash: approvalHash });
  ...
  const parseMilestonesFromForm = (milestonesInput: string[], deadlineDuration: number) => {
    const percBP: number[] = [];
  ...
  const { data: milestoneCountData } = useReadContract({
    ...
    args: viewId ? [BigInt(viewId)] : undefined,
    query: { enabled: !!viewId && !!agreementData && (agreementData as any)[24] === true },
  });

  // Fetch milestone rows when milestoneCount available
  const contractsForMilestones = useMemo(() => {
    const count = typeof milestoneCountData === 'bigint' ? Number(milestoneCountData) :
      typeof milestoneCountData === 'number' ? milestoneCountData : 0;
  ...
  const { data: createTokenDecimals } = useReadContract({
    address: isValidAddress(form.customTokenAddress) ? (form.customTokenAddress as `0x${string}`) : undefined,
    abi: ERC20_ABI.abi,
  ...
  useEffect(() => {
    if (typeof createTokenDecimals === "number" || typeof createTokenDecimals === "bigint") {
      setForm((prev) => ({ ...prev, tokenDecimals: Number(createTokenDecimals) }));
    }
  }, [createTokenDecimals]);
  ...
  const [createApprovalState, setCreateApprovalState] = useState({ isApprovingToken: false, needsApproval: false });
  ...
  useEffect(() => {
    ...
    const syncEscrowToBackend = async () => {
      if (!txSuccess || !txHash || txHash === lastSyncedTxHash || isSyncing || !syncDataRef.current) {
        return;
      }
      ...
      try {
        ...
        const onChainMetadata = {
          ...
          serviceRecipient: serviceRecipientAddr,
          token: tokenAddr,
          amount: form.amount,
          vestingMode,
          chainId: chainId,
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
        ...
        // Backend needs wallet addresses for both parties
        let firstPartyAddr: string;
        let counterPartyAddr: string;

        if (escrowType === "myself") {
          if (form.payer === "me") {
            // Current user is service recipient (pays), counterparty is service provider
            firstPartyAddr = serviceProviderAddr; // Service provider (counterparty)
            counterPartyAddr = serviceRecipientAddr; // Service recipient (me)
          } else {
            // Counterparty is service recipient (pays), current user is service provider
            firstPartyAddr = serviceProviderAddr; // Service provider (me)
            counterPartyAddr = serviceRecipientAddr; // Service recipient (counterparty)
          }
        } else {
          // Two other parties
          if (form.payerOther === "partyA") {
            // Party A is service recipient, Party B is service provider
            firstPartyAddr = serviceProviderAddr; // Service provider (Party B)
            counterPartyAddr = serviceRecipientAddr; // Service recipient (Party A)
          } else {
            // Party B is service recipient, Party A is service provider
            firstPartyAddr = serviceProviderAddr; // Service provider (Party A)
            counterPartyAddr = serviceRecipientAddr; // Service recipient (Party B)
          }
        }
        ...
        console.log('📋 Escrow agreement data for backend:', {
          ...
          visibility: form.type === 'private' ? AgreementVisibilityEnum.PRIVATE : AgreementVisibilityEnum.PUBLIC,
          includesFunds: true,
          secureTheFunds: true,
        });
        ...
        await agreementService.createAgreement(
          {
            ...
            firstParty: firstPartyAddr,
            counterParty: counterPartyAddr,
            deadline: deadline?.toISOString() as string,
            amount: parseFloat(form.amount),
            tokenSymbol: form.token === 'custom' ? 'custom' : form.token,
            contractAddress: form.token === 'custom' ? form.customTokenAddress : undefined,
        ...
        handleBackendSyncError(err, txHash!);
      } finally {
        setIsSyncing(false);
        resetWrite();
    ...
    const handleBackendSyncError = (err: any, txHash: string) => {
      const errorCode = err.response?.data?.error;
      const errorMessage = err.response?.data?.message;

      let userMessage = 'Failed to sync escrow to database';
      let userDescription = errorMessage || 'Please try again or contact support';
      ...
      switch (errorCode) {
        ...
        case 11: // SameAccount
          ...
          userDescription = 'Service provider and recipient cannot be the same account';
          break;

        case 12: // MissingWallet
          userMessage = 'Wallet not connected';
          userDescription = 'Your wallet must be connected to create escrow agreements';
          break;

        case 17: // Forbidden
          userMessage = 'Account restricted';
          userDescription = 'Your account is currently restricted from creating agreements';
          break;
      }
    ...
    syncEscrowToBackend();
  }, [txSuccess, txHash, lastSyncedTxHash, isSyncing, form.amount, form.description, form.evidence, form.type, form.title, form.token, form.customTokenAddress, form.payer, form.counterparty, form.payerOther, form.partyB, form.partyA, escrowType, deadline, address, resetWrite, loadEscrows, chainId]);
  ...
  const handleCreateAgreementOnChain = async () => {
    ...
    try {
      ...
      if (parsed.percBP.length > 0) {
        ...
        if (totalBP !== 10000) {
          setUiError("Milestone percentages must sum to 100% (10000 basis points)");
          return;
        }
      }
    } catch (err: any) {
    ...
    const callerIsDepositor = serviceRecipientAddr.toLowerCase() === address?.toLowerCase();
                  ...
                  >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mb-2">
                        Enter milestone lines as <code>percentBP:offsetSeconds</code> (e.g. <code>50:604800</code> meaning 50% at +7 days). Percent values are converted to basis points automatically.
                      </p>
                        ...
                        <div key={idx} className="mb-2 flex gap-2">
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              setForm({ ...form, milestones: next.length ? next : [""] });
                            }}
                          >
                            Remove
                          </Button>
                    ...
                    <div className="mt-6 flex justify-end gap-3 border-t border-white/10 pt-3">
                      <Button
                        ...
                        disabled={isSubmitting || isTxPending || isApprovalPending}
                      >
                        Save Draft
                      </Button>
                      <Button
                      ...
                      >
                        {isSubmitting || isTxPending || isApprovalPending ? (
                          <>
                            ...
                            {createApprovalState.isApprovingToken ? "Approving..." : "Creating..."}
                          </>
                        ) : (
                          <>
        ...
        {listed.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
            {listed.map((e) => (
              <div
                key={e.id}
                className={`rounded-xl p-5 ring-2 ring-white/10 ${e.status === "completed"
