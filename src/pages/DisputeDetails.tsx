...
export default function DisputeDetails() {
  ...
  const handleStartVote = useCallback(async (probono: boolean) => {
    try {
      ...
      const fee = probono ? BigInt(0) : BigInt(parseEther(FEE_AMOUNT).toString());
  ...
  return (
    <div className="animate-fade-in space-y-6 py-6 text-white">
      ...
      <div className="flex gap-3 border-b border-white/10 py-6">
        ...
        {dispute.status === "Pending" && isCurrentUserPlaintiff() && dispute.agreement?.type === 2 && (
          <Button
            variant="outline"
        ...
        {dispute.status === "Pending" && isCurrentUserPlaintiff() && dispute.agreement?.type === 1 && (
          <Button
            variant="outline"
            className="border-green-400/30 text-green-300 hover:bg-green-500/10"
            onClick={() => setSettleModalOpen(true)}
